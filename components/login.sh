...
cd /home/ubuntu && rm -rf login && unzip -o /tmp/login.zip &>>$LOG && mv login-main login && cd /home/ubuntu/login && export GOPATH=/go && export PATH=$PATH:$GOROOT/bin:$GOPATH/bin  go build
Stat $?
